...
int main(int argc, char **argv)
{
  ...
  fprintf(stderr, "************************\n");
  fprintf(stderr, "SCREEN INFO : %dx%dx%d\n", videoInfo->current_w, videoInfo->current_h, videoInfo->vfmt->BitsPerPixel);
  fprintf(stderr, "************************\n");
  surface = SDL_SetVideoMode(videoInfo->current_w, videoInfo->current_h, videoInfo->vfmt->BitsPerPixel,
                             SDL_HWSURFACE|
  ...
  if(systemColorDepth != 16 && systemColorDepth != 32) {
    fprintf(stderr,"Unsupported color depth '%d'.\nOnly 16 and 32 bit color depths are supported\n", systemColorDepth);
    exit(-1);
  }
...
template <typename T> inline void coleman_stretcher(u8 *src, u8 *dest,
  int srcWidth, int srcHeight, int srcPitch,
  int destWidth, int destHeight, int destPitch)
{
  // Stretch to full screen at any time.

  if (! regened)
  {
    int h_stretch_x = destHeight / srcHeight;
    int h_stretch_remain = destHeight % srcHeight;
    int h_stretch_add_in_sixteen = (16 * h_stretch_remain) / srcHeight;

    int w_stretch_x = destWidth / srcWidth;
    int w_stretch_remain = destWidth % srcWidth;
    int w_stretch_add_in_sixteen = (16 * w_stretch_remain) / srcWidth;

    int w_regen_orders[16] = {0, 8, 4, 12, 2, 10, 6, 14, 1, 9, 5, 13, 3, 11, 7, 15};
    int h_regen_orders[16] = {0, 8, 4, 12, 2, 10, 6, 14, 1, 9, 5, 13, 3, 11, 7, 15};

    int q;
    for(q = 0; q < 16; q++)
    {
      h_regen[q] = h_stretch_x;
    }

    for(q = 0; q < h_stretch_add_in_sixteen; q++)
    {
      h_regen[h_regen_orders[q] ]++;
    }

    for(q = 0; q < 16; q++)
    {
      w_regen[q] = w_stretch_x;
    }

    for(q = 0; q < w_stretch_add_in_sixteen; q++)
    {
      w_regen[w_regen_orders[q] ]++;
    }
    regened = true;
  }
  ...
  {
    for(int j = 0; j < h_regen[i % 16]; j++)
    {

      // The legacy SDL_CALL_STRETCHER..
      // Stretch one row.
      T *s = (T *)src;
      T *d = (T *)dest;
      for(int k = 0; k < srcWidth; k++) {
        for(int l = 0; l < w_regen[k % 16]; l++)
        {
          *d++ = *s;
        }
        *s++;
...
void systemDrawScreen()
{
  ...
  {
    case 16:
    coleman_stretcher<u16>(src, dest, srcWidth, srcHeight, srcPitch, surface->w, surface->h, destPitch);
    break;
    case 32:
    default:
    coleman_stretcher<u32>(src, dest, srcWidth, srcHeight, srcPitch, surface->w, surface->h, destPitch);
    break;
  }
